...
import { Plus, Search, Store, Users, TrendingUp, Settings, ChevronRight, X, Check, Globe, Phone, Mail, Palette, LogOut } from 'lucide-react'
...
export default function SuperAdminDashboard({ restaurants: initialRestaurants, stats }: Props) {
  const [tab, setTab] = useState<'restaurants' | 'stats'>('restaurants')
  ...
  // Realtime — nouveau resto sans refresh
  useEffect(() => {
  ...
  const activeRestos = localRestaurants.filter(r => r.is_active).length
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="grid grid-cols-3 gap-3 px-5 py-4">
        {[
          { label: 'Restaurants', value: String(localRestaurants.length), sub: `${activeRestos} actifs`, icon: Store, color: '#F26522' },
          { label: 'CA Total', value: formatPrice(totalRevenue, 'XOF'), sub: 'Toutes commandes', icon: TrendingUp, color: '#22C55E' },
          { label: 'Tables actives', value: '—', sub: 'En ce moment', icon: Users, color: '#3B82F6' },
        ].map((s, i) => (
      ...
      <div className="px-5 space-y-3 pb-8">
        <div className="flex items-center justify-between mb-1">
          <p className="font-bold text-gray-900 text-sm">{filtered.length} restaurant{filtered.length > 1 ? 's' : ''}</p>
        </div>
        <AnimatePresence>
          {filtered.map((r, i) => (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                    ...
                    r.plan === 'pro' ? 'bg-blue-100 text-blue-600' :
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            <p className="text-4xl mb-2"><TwemojiIcon emoji="🏪" size={20} /></p>
  ...
  async function create() {
    ...
    try {
      const res = await fetch('/api/restaurants', {
        ...
        body: JSON.stringify({ ...form, is_active: true, module_social: true, module_games: true, module_delivery: true, module_loyalty: true, module_birthday: true, tax_rate: 0 }),
      })
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div className="flex items-center justify-between p-5 pb-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Étape {stepIdx + 1}/3</p>
          </div>
                ...
                <div className="bg-white rounded-xl p-3 space-y-2 border border-gray-100">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-black px-3 py-1 rounded-xl"
                      style={{ backgroundColor: '#FFF7F0', color: '#F26522' }}>{credentials.password}</span>
                  </div>
                  <div className="h-px bg-gray-100" />
                  <div className="flex justify-between items-center">
              ...
              <div className="space-y-5">
                ...
                {[
                  ...
                  <div key={c.key}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input type="text" value={(form as any)[c.key]}
                        onChange={e => set(c.key, e.target.value)}
                      ...
                      <div className="w-12 h-12 rounded-2xl flex-shrink-0"
                        style={{ backgroundColor: (form as any)[c.key] }} />
                    </div>
                  </div>
                ))}
                {/* Preview */}
                <div className="rounded-3xl p-4" style={{ backgroundColor: form.primary_color + '10' }}>
                  ...
                  <div className="flex gap-2 mt-3">
                    <div className="px-3 py-1.5 rounded-xl text-xs font-bold text-white" style={{ backgroundColor: form.primary_color }}>Menu</div>
                    <div className="px-3 py-1.5 rounded-xl text-xs font-bold text-white" style={{ backgroundColor: form.secondary_color }}>Jeux</div>
                    <div className="px-3 py-1.5 rounded-xl text-xs font-bold text-white" style={{ backgroundColor: form.accent_color }}>Match</div>
                  </div>
                </div>
              </div>
            )}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div className="h-24 rounded-t-[2rem] flex items-end px-5 pb-0 relative"
          ...
          <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center font-black text-2xl translate-y-8 shadow-lg"
            style={{ color: p }}>
            {restaurant.logo_url
        ...
        <div className="px-5 pt-12 pb-6">
          <div className="flex items-start justify-between mb-1">
            ...
            <span className={`text-xs px-2.5 py-1 rounded-full font-bold mt-1 ${restaurant.is_active ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}>
          ...
          <div className="flex gap-2 mt-3 mb-5">
            ...
            <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-500">Bot: {restaurant.bot_name}</span>
          </div>
